const animes = require('../python/data/anime.json');
const keyword_extractor = require("keyword-extractor");
const fs = require('fs')

function getKeyWords(text){
    const data = require('../python/data/keywords_nodisplay.json');
    var real = [];
    ...
    Object.keys(animes).forEach((key, index) => {
        Object.keys(animes[key]).forEach((anime, i) => {
            ...
            if(Object.keys(animes).length -1 == index && Object.keys(animes[key]).length - 1 == i) {
                fs.writeFileSync('./python/data/keywords_nodisplay.json', JSON.stringify(keywords))
            }
        })
    })
}
